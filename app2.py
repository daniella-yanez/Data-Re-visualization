...
hover_tpl = (
    ...
    "<i style='color:#c8a028'>%{customdata[8]}</i>"
    "<extra></extra>"
)
...
if sel_country != "— All countries —":
    ...
    us_connection = any(kw in row["key_context"].lower()
    ...
    if show_us_link:
        ...
        story_df = story_df[story_df["key_context"].str.lower().apply(
            lambda t: any(kw in t for kw in us_kw)
        )]
    ...
    for _, row in story_df.iterrows():
        ptype = row["pathway_type"]
        us_link = any(kw in row["key_context"].lower()
                      for kw in ["us ", "u.s.", "american", "united states", "foreign policy",
